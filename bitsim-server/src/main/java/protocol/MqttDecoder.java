...
public class MqttDecoder extends ReplayingDecoder<MqttDecoder.DecoderState> {
    ...
    private Result<PublishVariableHeader> decodePublishVariableHeader(ByteBuf byteBuf, FixedHeader fixedHeader) {
        ...
        PublishVariableHeader publishVariableHeader = new PublishVariableHeader(topicName, packetId);
     ...
     * 解码只有包ID的可变头部
     */
    private Result<PacketIdVariableHeader> decodePackageIdVariableHeader(ByteBuf byteBuf) {
        // 已解码字节
        int useNumOfBytes = 0;

        // 解析包ID
        int packetId = byteBuf.readUnsignedShort();
        // 校验包ID
        if (packetId == 0) {
            throw new DecoderException("无效的包ID" + packetId);
        }
        ...
        PacketIdVariableHeader packetIdVariableHeader = new PacketIdVariableHeader(packetId);

        return new Result<PacketIdVariableHeader>(packetIdVariableHeader, useNumOfBytes);
    }
